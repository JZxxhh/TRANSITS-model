...
# Model's version
version        = '5.0'
print('### This is TRANSITS model v' + version)
print('-------------------------------')
...
file = open("inputs_TUV/inputs_JNO2.txt","r") 
importJNO2 = np.fliplr(np.transpose(np.genfromtxt(file, skip_header=1, delimiter=""))) #no ozone difference
# Create object in the input object
inp.JNO2 = f.Ozone_column()
inp.JNO2.DU200 = importJNO2[1, :]
inp.JNO2.DU225 = importJNO2[2, :]
inp.JNO2.DU250 = importJNO2[3, :]
inp.JNO2.DU275 = importJNO2[4, :]
inp.JNO2.DU300 = importJNO2[5, :]
inp.JNO2.DU325 = importJNO2[6, :]
inp.JNO2.DU350 = importJNO2[7, :]
inp.JNO2.DU375 = importJNO2[8, :]
inp.JNO2.DU400 = importJNO2[9, :]
inp.JNO2.DU425 = importJNO2[10, :]
inp.JNO2.DU450 = importJNO2[11, :]
inp.JNO2.DU475 = importJNO2[12, :]
inp.JNO2.DU500 = importJNO2[13, :]
...
file = open("inputs_TUV/inputs_J14NO3_425DU.txt","r")
inp.J14NO3.DU425 = np.fliplr(np.transpose(np.genfromtxt(file, skip_header=1, delimiter="")))
file = open("inputs_TUV/inputs_J15NO3_425DU.txt","r")
inp.J15NO3.DU425 = np.fliplr(np.transpose(np.genfromtxt(file, skip_header=1, delimiter="")))
file = open("inputs_TUV/inputs_J14NO3_450DU.txt","r")
inp.J14NO3.DU450 = np.fliplr(np.transpose(np.genfromtxt(file, skip_header=1, delimiter="")))
file = open("inputs_TUV/inputs_J15NO3_450DU.txt","r")
inp.J15NO3.DU450 = np.fliplr(np.transpose(np.genfromtxt(file, skip_header=1, delimiter="")))
file = open("inputs_TUV/inputs_J14NO3_475DU.txt","r")
inp.J14NO3.DU475 = np.fliplr(np.transpose(np.genfromtxt(file, skip_header=1, delimiter="")))
file = open("inputs_TUV/inputs_J15NO3_475DU.txt","r")
inp.J15NO3.DU475 = np.fliplr(np.transpose(np.genfromtxt(file, skip_header=1, delimiter="")))
file = open("inputs_TUV/inputs_J14NO3_500DU.txt","r")
inp.J14NO3.DU500 = np.fliplr(np.transpose(np.genfromtxt(file, skip_header=1, delimiter="")))
file = open("inputs_TUV/inputs_J15NO3_500DU.txt","r")
inp.J15NO3.DU500 = np.fliplr(np.transpose(np.genfromtxt(file, skip_header=1, delimiter="")))
...
for run in L_runs:
    ...
    if len(L_runs) == 1:
        ...
        print('>>>>> Par1/' + run[1] +' = ' + str(run[2]) + ', Par2/' + str(run[3]) + ' = ' + str(run[4]) + ', Par3/' + str(run[5]) + ' = ' + str(run[6]) + ', Par4/' + str(run[7]) + ' = ' + str(run[8]) + ', Par5/' + str(run[9]) + ' = ' + str(run[10]) + ', Par6/' + str(run[11]) + ' = ' + str(run[12]))

    # Read inputs again
    f.Read_inputs(inp, param, input_file_to_read)

    ### Set values to each parameter in the general case :      
    param.nb_yr =                   3              # years                     number of computed years in the simulation (25 orig)
    param.nb_end =                  0              # week                       number of the last computed week
    ...
    param.SN_d =                    0.38           # adimensional             snow density
    ...
    param.Phi =                     0.002          #0.002           # molecules.photon^(-1)     (0.0028) photolytic quantum yield
    ...
    param.accu_mass_yr =            250            # kg.m^(-2).yr^(-1)         accumulated snow mass per year (change to 18 when snow, poss. 15),  0.00000001 otherwise
    param.FPI_mass_yr =             6.6e-6         # kgN.m^(-2).yr^(-1)        TOTAL primary input mass deposited per year 
    ...
    param.out_fric =                0.35           # export fraction
    ...
    option.dep_diff_scenario     =           1              # Diffusion option : 0 = 100% deposition on top layer + no diffusion at all,
    ...
    option.phi                   =         0                 # option = 1: use quantum yield value as given value; else use phi(T) relationship from Chu et al. (2003)
    ...
    if param.SN_set_height/param.photic_zone > param.SN_set_height:
...
# filename = 'ncoutput\%s.nc'%input('please input the filename\n')
