...
for infile in glob.glob( os.path.join(path, '*.txt') ):
    ...
    infile_name = re.findall(r'\d+', infile)[0] + 'DU'
    
    ### ACTINIC FLUX ::: Reading the input file : infile
    header = np.arange(0, 200, 1) * 1e-2
    
    inp = open(infile,"r")
    TUV_DU = np.genfromtxt(inp)
...
##    # Write data in J_NO2 header file
    ...
    for i in range(41):
        # Write the SZA value
        J_14NO3[i, 0] = J_15NO3[i, 0] = J_O3[i, 0] = J_NO2[i, 0] = 90-i 
        ...
        for d in range(cols):
            ...
            J_14NO3[i, d+1] = (I_abs[:, 0] * XS14_abs).sum() 
            J_15NO3[i, d+1] = (I_abs[:, 0] * XS15_abs).sum()
    
    # flipud the array (because sza in first col decrease from 90 to 50, while J increase from 50 t0 90)
...
# Eventually merge the two files and delete the intermediate ones
with open('inputs_JO3.txt','w') as f1:
    with open('JO3_header.txt','r') as f2:
        with open('JO3_TEMP.txt','r') as f3:
            f1.write(f2.read())
            f1.write(f3.read())
...
with open('inputs_JNO2.txt','w') as f1:
    with open('JNO2_header.txt','r') as f2:
        with open('JNO2_TEMP.txt','r') as f3:
            f1.write(f2.read())
            f1.write(f3.read())
