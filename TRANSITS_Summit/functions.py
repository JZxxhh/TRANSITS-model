...
def Read_inputs(i, p, filename):
    ...
    i.BC            = np.zeros((p.nb_ts))   # ng/g                  mean BC deposition concentration ,including wet and dry deposition
    print(nb_rows)
    for j in range(3, p.nb_ts+3):
        i.accu_mass_rep[j-3] = ws.cell(j, 2).value
        i.AT_conc[j-3]       = ws.cell(j, 3).value
        i.AT_height[j-3]     = ws.cell(j, 4).value
        i.AT_Temp[j-3]       = ws.cell(j, 5).value
        i.AT_Pres[j-3]       = ws.cell(j, 6).value
        i.AT_O3_conc[j-3]    = ws.cell(j, 7).value
        i.AT_D17O_O3b[j-3]   = ws.cell(j, 8).value
        i.AT_f_exp[j-3]      = ws.cell(j, 9).value
        i.FS_mass_rep[j-3]   = ws.cell(j, 10).value
        i.FS_d15N[j-3]       = ws.cell(j, 11).value
        i.FS_D17O[j-3]       = ws.cell(j, 12).value
        i.FT_mass_rep[j-3]   = ws.cell(j, 13).value
        i.FT_d15N[j-3]       = ws.cell(j, 14).value
        i.FT_D17O[j-3]       = ws.cell(j, 15).value
        i.O3colDU[j-3]       = ws.cell(j, 16).value
        i.SZA_mean[j-3]      = ws.cell(j, 17).value
        # i.BC[j-2]            = ws.cell_value(j, 17)
    i.calc_AT_mass(p)
    ...
    def __init__(self, p, i):
        ...
        for nbl in range(len(index)-1):
            ...
            for nb in range(index1,index2):
                ...
                self.SN_date[nb] = -1
                self.SN_year[nb] = -1
                self.SN_mass[nb] = p.SN_set_thick*(p.M_N/p.M_NO3) * p.SN_rho * inco[nbl,1] * 1000 *1e-12
                self.SN_d15N[nb] = inco[nbl,2]
                self.SN_D17O[nb] = inco[nbl,3]        
...
def create_ncfile(filename,data1,data2,paraml):
    if filename == 'ncoutput\0.nc':
        ...
        try:
            ...
            filename = 'ncoutput\figure\%s.nc'%input('please input the filename\n')
            create_ncfile(filename,data1,data2,paraml)
        else:
