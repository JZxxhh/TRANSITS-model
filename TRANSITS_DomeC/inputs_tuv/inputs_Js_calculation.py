...
E          = 1/wl * 1e7
X          = (E - C) / W
XSoverE    = (1000*log(10)/N_A) * factor * A * (1 - S*X) * exp(-X*X * (1 - S*X + 0.5*(S*X)*(S*X)))
XS14_abs   = XSoverE * E
...
XSO3_nm  = new_nm
XSO3_abs = interp(new_nm, XSO3[:, 0], XSO3[:, 1])
#########################################################################################

path= "raw_TUV_outputs_(Actinic_Fluxes)"

### Create output of the J_O3 calculation
J_O3 = np.zeros((41, len(glob.glob( os.path.join(path, '*.txt') )) + 1))

### Create output file for the J_O3 calculation
JO3_header = file("JO3_header.txt", "w" )
...
for infile in glob.glob( os.path.join(path, '*.txt') ):
    ...
    inp = open(infile, "r")
    read_for_header = np.genfromtxt(inp, skip_header=0, delimiter="")
    header = read_for_header[0, :]      # This is only to read the header (= depths)
    inp = open(infile,"r")
    TUV_DU = np.genfromtxt(inp, skip_header=1, delimiter="")
    ...
    file('inputs_J14NO3_' + infile_name +'.txt','wb').write(file('header.txt','rb').read()+file('data14.txt','rb').read())
    file('inputs_J15NO3_' + infile_name +'.txt','wb').write(file('header.txt','rb').read()+file('data15.txt','rb').read())
...
### OUTPUT EXPORT in filr for the 310 nm extraction
# save data in temporary file
np.savetxt('JO3_TEMP.txt', J_O3, delimiter="\t")

# Eventually merge the two files and delete the intermediate ones
file('inputs_JO3.txt','wb').write(file('JO3_header.txt','rb').read()+file('JO3_TEMP.txt','rb').read())
